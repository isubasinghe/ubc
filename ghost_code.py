...
def insert_single_succ_node_on_edge(
    ...
    assert before_name in nodes or before_name in (
        source.NodeNameErr,
        source.NodeNameRet,
    ), f"inserting before imaginary node"
...
def sprinkle_call_assert_preconditions(
    fn: nip.Function,
    name: source.NodeName,
    precond: source.ExprT[source.ProgVarName | nip.GuardVarName],
) -> Iterable[Insertion]:
    for pred in fn.cfg.all_preds[name]:
        yield Insertion(
            node_name=source.NodeName(f"prove_{pred}_{name}"),
            after=pred,
            before=name,
            kind=K.NODE_PRE_CONDITION_OBLIGATION_FNCALL,
            expr=precond,
        )


def sprinkle_call_assume_postcondition(
    name: source.NodeName,
    node: source.NodeCall[Any],
    postcond: source.ExprT[source.ProgVarName | nip.GuardVarName],
) -> Insertion:
    return Insertion(
        node_name=source.NodeName(f"assume_postcond_{name}_{node.succ}"),
        after=name,
        before=node.succ,
        kind=K.NODE_ASSUME_POST_CONDITION_FNCALL,
        expr=postcond,
    )
    ...
    def f(v: source.ExprVarT[source.HumanVarName]) -> source.ExprT[source.VarNameKind]:
        if isinstance(v.name.subject, str) and is_global_smt(v.name.subject):
            e = source.ExprVar(
                ...
                source.ProgVarName(v.name.subject),
            )
...
def sprinkle_call_conditions(
    filename: str,
    fn: nip.Function,
    ctx: Dict[str, source.FunctionSignature[source.ProgVarName]],
) -> Iterator[Insertion]:
