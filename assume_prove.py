...
def convert_expr_dsa_vars_to_ap(
    ...
    if isinstance(expr, source.ExprNum):
        ...
        return source.ExprFunction(
            expr.typ,
            expr.function_name,
            [convert_expr_dsa_vars_to_ap(arg) for arg in expr.arguments],
        )
    assert_never(expr)
...
def apply_incarnation_for_node(
    func: dsa.Function,
    n: source.NodeName,
    prog_var: source.ExprVarT[source.ProgVarName | nip.GuardVarName],
) -> APVar:
