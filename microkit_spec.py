...
SchedState = source.TypeBitVec(1)
...
lc_schedstate = source.FunctionName('lc_schedstate')
...
BoundTCB = source.FunctionName('BoundTCB')
BoundNotification = source.FunctionName('BoundNotification')
...
PlatformContext = source.TypeBitVec(408)
...
lc_err = source.ExprNum(source.TypeBitVec(408), 0xdead1c)
...
lc_progvar = source.ExprVar(typ=source.TypeBitVec(
    408), name=source.ProgVarName("local_context#ghost"))
...
def platform_context_update(
    ...
    *,
    ...
    lc_schedstate_val: source.ExprT[source.ProgVarName] | None = None
    ...
    fields = [lc_running_pd, lc_receive_oracle, lc_unhandled_notified,
              lc_last_handled_notified, lc_unhandled_ppcall, lc_unhandled_reply, lc_last_handled_reply, lc_schedstate]
    vals = [lc_running_pd_val, lc_receive_oracle_val, lc_unhandled_notified_val, lc_last_handled_notified_val,
            lc_unhandled_ppcall_val, lc_unhandled_reply_val, lc_last_handled_reply_val, lc_schedstate_val]
    typs = [PD, NextRecv, Set_Ch, Set_Ch,
            Maybe_Prod_Ch_MsgInfo, Maybe_MsgInfo, Maybe_MsgInfo, SchedState]
...
lhave_signal = source.ExprVar(
    source.type_word8, source.ProgVarName("lhave_signal___char#v"))


def nbsendrecv_postcondition(arg_lc: source.ExprVarT[source.ProgVarName], ret_lc: source.ExprVarT[source.ProgVarName], ret_var: source.ExprVarT[source.ProgVarName]) -> source.ExprT[source.ProgVarName]:
...
functions_spec = {
    ...
    "tmp.seL4_ReplyRecv": source.Ghost(
        precondition=conjs(
    ...
    "tmp.seL4_NBSendRecv": source.Ghost(
        precondition=conjs(
            neq(source.ExprFunction(NextRecv, lc_receive_oracle, (arg(lc_progvar),)),
                source.ExprFunction(NextRecv, NR_Unknown, ())),
            neq(source.ExprFunction(NextRecv, lc_receive_oracle, (arg(lc_progvar),)),
            ...
            eq(source.ExprFunction(Maybe_MsgInfo, lc_unhandled_reply, (arg(lc_progvar),)),
               source.ExprFunction(Maybe_MsgInfo, MsgInfo_Nothing, [])),
            eq(source.ExprFunction(SchedState, lc_schedstate, (arg(lc_progvar), )),
               source.ExprFunction(SchedState, BoundTCB, (),)),
        ),
        postcondition=conjs(nbsendrecv_postcondition(
            arg(lc_progvar), lc_progvar, msg_info_ret)),
        loop_invariants={},
        loop_iterations={}
    ),
    "tmp.handler_loop": source.Ghost(
        loop_invariants={
            lh(handler_loop_node_name()): conjs(
                source.expr_implies(
                    neq(lhave_signal, char(0)),
                    conjs(
                        eq(have_reply, char(0)),
                        eq(
                            source.ExprFunction(
                                SchedState, lc_schedstate, (lc_progvar,)),
                            source.ExprFunction(SchedState, BoundTCB, (),)
                        )
                    )
                ),
