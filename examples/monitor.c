...
{return seL4_NoError;}
static seL4_SchedContext_Consumed_t
...
seL4_Word bootstrap_invocation_data[110];
...
static unsigned perform_invocation(seL4_Word *invocation_data, unsigned offset, seL4_Word idx)
{
...
static void monitor(void)
{
    for (;;) {
        ...
        if (label == seL4_Fault_NullFault && badge < 64) {
      ...
      if(cont==0) {
        if (badge < 64 && pd_names[badge][0] != 0) {
            puts(STRINGLITERAL);
            puts(pd_names[badge]);
            puts(STRINGLITERAL);
        } else {
        ...
        err = seL4_TCB_ReadRegisters(tcb_cap, 
                                             0
                                                  , 0, sizeof(seL4_UserContext) / sizeof(seL4_Word), &gregs);
        regs = gregs;
        if (err != seL4_NoError) {
            fail(STRINGLITERAL);
        }
...
void main(seL4_BootInfo *bi)
{
    ...
    unsigned offset = 0;
    for (seL4_Word idx = 0; idx < bootstrap_invocation_count; idx++) {
        offset = perform_invocation(bootstrap_invocation_data, offset, idx);
    }
    offset = 0;
    for (seL4_Word idx = 0; idx < system_invocation_count; idx++) {
