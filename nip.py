...
class GenericFunction(source.GenericFunction[source.VarNameKind,
                                             source.VarNameKind2]):
    """
...
Function = GenericFunction[source.ProgVarName | GuardVarName,
                           source.ProgVarName | GuardVarName]
...
def var_deps(
        expr: source.ExprT[source.ProgVarName]) -> source.ExprT[GuardVarName]:
    # for now, we ignore short circuiting
    # if a = b + c, returns a#assigned = b#assigned && c#assigned
    return reduce(source.expr_and, map(guard_var,
                                       source.all_vars_in_expr(expr)),
                  source.expr_true)
...
def make_state_update_for_node(
    ...
    if isinstance(node, source.NodeBasic):
        ...
        deps = reduce(source.expr_and, (var_deps(arg) for arg in node.args),
                      source.expr_true)
        ...
        assert not isinstance(
            node,
            source.NodeEmpty | source.NodeCond | source.NodeAssume
            | source.NodeAssert,
...
def make_protection_for_node(
    node: source.Node[source.ProgVarName], ) -> source.ExprT[GuardVarName]:
    # for now, we ignore short circuiting
    return reduce(
        source.expr_and,
        (guard_var(v) for v in source.used_variables_in_node(node)
...
def make_initial_state(
        func: source.Function) -> Iterator[source.Update[GuardVarName]]:
    # TODO: globals
...
def update_node_successors(
        node: source.Node[source.VarNameKind],
        successors: Sequence[source.NodeName]
    ...
    if isinstance(node, source.NodeCond):
        ...
        return dataclasses.replace(node,
                                   succ_then=successors[0],
                                   succ_else=successors[1])
...
def unify_variables_to_make_ghost(
    ...
    conversion_map: defaultdict[
        ...
        list[source.ExprVarT[source.ProgVarName
                             | GuardVarName]], ] = defaultdict(lambda: [])
    ...
    for var in all_vars:
        ...
        conversion_map[source.ExprVar(
            var.typ, source.HumanVarName(prefix, path=(),
                                         use_guard=False))].append(var)
        conversion_map[source.ExprVar(
            source.type_bool,
            source.HumanVarName(prefix, path=(),
                                use_guard=True))].append(guard_var(var))
    ...
    def converter(
        ...
        if isinstance(human.name.subject, str) and is_global_smt(
                human.name.subject):
    ...
    return source.Ghost(
        precondition=source.convert_expr_vars(converter,
                                              func.ghost.precondition),
        postcondition=source.convert_expr_vars(postcondition_converter,
                                               func.ghost.postcondition),
        loop_invariants={
...
def nip(func: source.Function) -> Function:
    ...
    for n in func.traverse_topologically(skip_err_and_ret=True):
        node = func.nodes[n]
        if isinstance(node,
                      source.NodeBasic | source.NodeCall | source.NodeCond):
    ...
    for n in func.traverse_topologically():
        ...
        if n in state_updates:
            ...
            # successors
            assert isinstance(
                node, source.NodeBasic | source.NodeCall
                | source.NodeEmpty), f"{type(node)}"
            assert len(jump_to) == 1
            update_name = source.NodeName(f"upd_n{n}")
            new_nodes[update_name] = NodeStateUpdate(state_updates[n],
                                                     jump_to[0])
