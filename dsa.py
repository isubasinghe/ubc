...
class GenericFunction(
    ghost_code.GenericFunction[source.VarNameKind, source.VarNameKind2]
):
    """DSA Function"""
...
Function = GenericFunction[
    Incarnation[source.ProgVarName | nip.GuardVarName],
    source.ProgVarName | nip.GuardVarName,
]
...
def unpack_dsa_var_name(
    v: Incarnation[BaseVarName],
) -> tuple[BaseVarName, IncarnationNum]:
    return v.base, v.inc
...
def make_dsa_var(
    v: source.ExprVarT[BaseVarName], inc: IncarnationNum
) -> source.ExprVarT[Incarnation[BaseVarName]]:
    return source.ExprVar(v.typ, Incarnation(v.name, inc))


def guard_var_at_node(
    func: Function, n: source.NodeName, var: Var[source.ProgVarName]
) -> Var[nip.GuardVarName]:
...
class DSABuilder:
    ...
    dsa_nodes: dict[
        source.NodeName, source.Node[Incarnation[nip.GuardVarName | source.ProgVarName]]
    ]
    """
...
def apply_incarnations(
    context: Mapping[
        source.ExprVarT[source.ProgVarName | nip.GuardVarName], IncarnationNum
    ],
...
def apply_insertions(s: DSABuilder) -> None:
    ...
    for node_name, node_insertions in s.insertions.items():
        for pred_name in s.original_func.acyclic_preds_of(node_name):
            updates: list[
                source.Update[Incarnation[source.ProgVarName | nip.GuardVarName]]
            ] = []
            for prog_var, new_incarnation_number in node_insertions.items():
                ...
                updates.append(
                    source.Update(
                        ...
                        source.ExprVar(
                            prog_var.typ,
                            name=Incarnation(prog_var.name, old_incarnation_number),
                        ),
                    )
                )
            if len(updates) == 0:
            ...
            incarnation_at_joiner_node: dict[
                source.ExprVarT[source.ProgVarName | nip.GuardVarName], IncarnationNum
            ] = dict(s.incarnations[pred_name])
            for upd in updates:
                incarnation_at_joiner_node[get_base_var(upd.var)] = upd.var.name.inc
...
def recompute_loops_post_dsa(
    ...
    assert (
        all_loop_nodes.keys() == s.original_func.loops.keys()
    ), "loop headers should remain the same through DSA transformation"

    loops: dict[source.LoopHeaderName, source.Loop[Incarnation[BaseVarName]]] = {}
...
def dsa(func: ghost_code.Function) -> Function:
    ...
    dsa_args: list[
        source.ExprVarT[Incarnation[source.ProgVarName | nip.GuardVarName]]
    ] = []
    ...
    dsa_loop_targets: dict[
        source.LoopHeaderName, tuple[Var[source.ProgVarName | nip.GuardVarName], ...]
    ] = {}
    for current_node in func.traverse_topologically():
        ...
        added_incarnations: dict[
            source.ExprVarT[source.ProgVarName | nip.GuardVarName],
            Var[source.ProgVarName | nip.GuardVarName],
        ] = {}
        ...
        if isinstance(node, source.NodeBasic):
            ...
            for upd in node.upds:
                ...
                assert (
                    upd.var not in added_incarnations
    ...
    return Function(
        cfg=cfg,
        signature=source.FunctionSignature(tuple(dsa_args), func.signature.returns),
        loops=loops,
        name=func.name,
        nodes=s.dsa_nodes,
