...
class GenericFunction(ghost_code.GenericFunction[source.VarNameKind,
                                                 source.VarNameKind2]):
    """DSA Function"""

    contexts: Mapping[source.NodeName,
                      Mapping[source.ExprVarT[source.ProgVarName
                                              | nip.GuardVarName],
                              IncarnationNum], ]
...
Function = GenericFunction[Incarnation[source.ProgVarName | nip.GuardVarName],
                           source.ProgVarName | nip.GuardVarName, ]


@dataclass(frozen=True)
class NodeJoiner(source.NodeBasic[Incarnation[source.ProgVarName
                                              | nip.GuardVarName]]):
    pass


def unpack_dsa_var_name(
    v: Incarnation[BaseVarName], ) -> tuple[BaseVarName, IncarnationNum]:
    return v.base, v.inc


def unpack_dsa_var(
    var: Var[BaseVarName],
) -> tuple[source.ExprVarT[BaseVarName], IncarnationNum]:
...
def make_dsa_var(
        v: source.ExprVarT[BaseVarName],
        inc: IncarnationNum) -> source.ExprVarT[Incarnation[BaseVarName]]:
    return source.ExprVar(v.typ, Incarnation(v.name, inc))


def guard_var_at_node(func: Function, n: source.NodeName,
                      var: Var[source.ProgVarName]) -> Var[nip.GuardVarName]:
...
class DSABuilder:
    ...
    dsa_nodes: dict[source.NodeName,
                    source.Node[Incarnation[nip.GuardVarName
                                            | source.ProgVarName]]]
    """
...
def apply_incarnations(
    context: Mapping[source.ExprVarT[source.ProgVarName | nip.GuardVarName],
                     IncarnationNum],
    root: source.ExprT[BaseVarName],
) -> source.ExprT[Incarnation[BaseVarName]]:
    ...
    if isinstance(root, source.ExprNum | source.ExprType | source.ExprSymbol):
        ...
        var: source.ExprVarT[source.ProgVarName
                             | nip.GuardVarName] = source.ExprVar(
                                 root.typ, root.name)
        ...
        return source.ExprOp(
            root.typ,
            source.Operator(root.operator),
            operands=tuple(
                apply_incarnations(context, operand)
        ...
        return source.ExprFunction(
            root.typ,
            root.function_name,
            tuple(apply_incarnations(context, arg) for arg in root.arguments),
        )
    assert_never(root)


def get_next_dsa_var_incarnation_number(
    s: DSABuilder,
    current_node: source.NodeName,
    var: source.ExprVarT[source.ProgVarName | nip.GuardVarName],
) -> IncarnationNum:
...
def get_next_dsa_var_incarnation_number_from_context(
    s: DSABuilder,
    context: Mapping[source.ExprVarT[source.ProgVarName | nip.GuardVarName],
                     IncarnationNum],
    var: source.ExprVarT[source.ProgVarName | nip.GuardVarName],
...
def apply_insertions(s: DSABuilder) -> None:
    ...
    for node_name, node_insertions in s.insertions.items():
        for pred_name in s.original_func.acyclic_preds_of(node_name):
            updates: list[source.Update[Incarnation[source.ProgVarName
                                                    | nip.GuardVarName]]] = []
            for prog_var, new_incarnation_number in node_insertions.items():
                ...
                updates.append(
                    source.Update(
                        make_dsa_var(prog_var, new_incarnation_number),
                        source.ExprVar(
                            prog_var.typ,
                            name=Incarnation(prog_var.name,
                                             old_incarnation_number),
                        ),
                    ))
            if len(updates) == 0:
                continue
            ...
            incarnation_at_joiner_node: dict[
                source.ExprVarT[source.ProgVarName | nip.GuardVarName],
                IncarnationNum] = dict(s.incarnations[pred_name])
            for upd in updates:
...
def recompute_loops_post_dsa(
    s: DSABuilder,
    dsa_loop_targets: Mapping[source.LoopHeaderName, tuple[Var[BaseVarName],
                                                           ...]],
    new_cfg: abc_cfg.CFG,
) -> Mapping[source.LoopHeaderName, source.Loop[Incarnation[BaseVarName]]]:
    abc_cfg.assert_single_loop_header_per_loop(new_cfg)

    # loop header => loop nodes
    all_loop_nodes: dict[source.LoopHeaderName, tuple[source.NodeName,
                                                      ...]] = {}
    for back_edge in new_cfg.back_edges:
        ...
        assert all(
            loop_header in new_cfg.all_doms[n] for n in loop_nodes
        ), "the loop header should dominate all the nodes in the loop body"
    ...
    for loop_header, loop_nodes in all_loop_nodes.items():
        assert set(s.original_func.loops[loop_header].nodes).issubset(
            loop_nodes
        ), "dsa only inserts joiner nodes, all previous loop nodes should still be loop nodes"
        loops[loop_header] = source.Loop(
            back_edge=s.original_func.loops[loop_header].back_edge,
...
def dsa(func: ghost_code.Function) -> Function:
    ...
    s = DSABuilder(original_func=func,
                   insertions={},
                   dsa_nodes={},
                   incarnations={})

    entry_context: dict[source.ExprVarT[source.ProgVarName | nip.GuardVarName],
                        IncarnationNum] = {}
    ...
    assert len(set(unpack_dsa_var_name(arg.name)[0]
                   for arg in dsa_args)) == len(
    ...
    for current_node in func.traverse_topologically():
        ...
        # TODO: clean this up
        context: dict[source.ExprVarT[source.ProgVarName | nip.GuardVarName],
                      IncarnationNum]
        curr_node_insertions: dict[source.ExprVarT[source.ProgVarName
                                                   | nip.GuardVarName],
        ...
        if current_node == func.cfg.entry:
            ...
            all_variables: set[source.ExprVarT[
                source.ProgVarName | nip.GuardVarName]] = set_union(
                    set(s.incarnations[p].keys())
                    for p in s.original_func.acyclic_preds_of(current_node))

            for var in all_variables:
                possibilities = set(
                    s.incarnations[p][var]
                    for p in s.original_func.acyclic_preds_of(current_node)
                    if var in s.incarnations[p])
        ...
        if loop_header := func.is_loop_header(current_node):
            ...
            for target in func.loops[loop_header].targets:
                ...
                targets.append(make_dsa_var(target, fresh_incarnation_number))
        ...
        if isinstance(node,
                      ghost_code.NodePostConditionProofObligation) and False:
        ...
        if isinstance(node, source.NodeBasic):
            ...
            for upd in node.upds:
                ...
                assert (upd.var not in added_incarnations
                        ), "duplicate updates in BasicNode"
            ...
            args = tuple(apply_incarnations(context, arg) for arg in node.args)
    ...
    return Function(
        cfg=cfg,
        signature=source.FunctionSignature(tuple(dsa_args),
