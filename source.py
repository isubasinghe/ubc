...
def expr_where_vars_are_used_in_node(
    node: Node[VarNameKind], selection: Set[ExprVarT[VarNameKind]]
) -> Iterator[tuple[ExprVarT[VarNameKind], ExprT[VarNameKind]]]:
...
def convert_expr(expr: syntax.Expr) -> ExprT[ProgVarName]:
    ...
    if expr.kind == "Op":
        return ExprOp(
            typ,
            Operator(expr.name),
            tuple(convert_expr(operand) for operand in expr.vals),
        )
    elif expr.kind == "Var":
...
def condition_to_evaluate_subexpr_in_expr(
    ...
    if isinstance(expr, ExprNum):
        ...
        reachability_condition = [
            condition_to_evaluate_subexpr_in_expr(operand, sub)
            for operand in expr.operands
        ]
...
class GhostlessFunction(Generic[VarNameKind, VarNameKind2]):
    ...
    def with_ghost(
        self, ghost: Ghost[HumanVarName] | None
    ) -> GenericFunction[VarNameKind, HumanVarName]:
        if ghost is None:
            ghost = Ghost(
...
def convert_function_nodes(
    nodes: Mapping[str | int, syntax.Node]
) -> Mapping[NodeName, Node[ProgVarName]]:
    ...
    for name, node in nodes.items():
        ...
        if node.kind == "Basic":
            ...
            safe_nodes[name] = NodeCond(
                succ_then=NodeName(str(node.left)),
                succ_else=NodeName(str(node.right)),
                expr=convert_expr(node.cond),
            )
        else:
